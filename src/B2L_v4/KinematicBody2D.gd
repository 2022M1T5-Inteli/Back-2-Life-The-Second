...
#Função do processo físico de gravidade.
	motion.y+= gravity

#Condições para fazer os personagens se moverem.
	if Input.is_action_pressed("ui_right"): #Movimento para a direita.
		motion.x = speed
		$Sprite.play("walking")
		$Sprite.flip_h = false
	elif Input.is_action_pressed("ui_left"): #Movimento para a esquerda.
		motion.x = -speed
		$Sprite.play("walking")
